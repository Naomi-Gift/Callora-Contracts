...
/// Hard cap on items per `batch_deduct` call.
/// Soroban's per-transaction CPU and memory limits bound how many items can be
/// processed safely. 50 keeps the transaction well within those limits.

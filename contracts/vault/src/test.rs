    ...
    fn run_sequence(seed: u64, max_deduct_val: i128, initial: i128, steps: usize) {
        ...
        for _ in 0..steps {
            ...
            match op {
                // --- deposit ---
                0 => {
                    // Keep fuzz deposits within approved reserve so failures are only from
                    // contract invariants, not token allowance/balance exhaustion.
                    let max_deposit = core::cmp::min(max_deduct_val, 1_000_000_i128);
